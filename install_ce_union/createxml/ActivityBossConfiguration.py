...
#生成xml文件
def ActivityBossConfiguration(channel, sid, ActivityBossIp, ActivityBossDbName, ActivityBossDbUser, ActivityBossDbPw):

    xmlpath = xml_path + '%s/%s/ActivityBossConfiguration.xml' % (channel, sid)
    ...
    name5B.setAttribute("value", "jdbc:mysql://%s:3306/%s?autoReconnect=true&amp;useUnicode=true&amp;characterEncoding=utf8" % (ActivityBossIp, ActivityBossDbName))
    name4B.appendChild(name5B)

    # 5C
    name5C=dom.createElement('property')
    name5C.setAttribute("name", "username")
    name5C.setAttribute("value", ActivityBossDbUser)
    name4B.appendChild(name5C)

    # 5D
    name5D=dom.createElement('property')
    name5D.setAttribute("name", "password")
    name5D.setAttribute("value", ActivityBossDbPw)
    name4B.appendChild(name5D)

    #3B1
    name3B1=dom.createElement('mapper')
    name3B1.setAttribute("resource","com/ft/ncp/dao/PlayerBossActivityInfoMapper.xml")
    ...
    print "ActivityBossConfiguration.xml 创建"
